...
function checkAnswer() {
    ...
    if (parseFloat(userAnswer) === parseFloat(currentExpression.answer)) {
        correctAnswers++;
        document.getElementById('status').textContent = `Score: ${correctAnswers} correct | ${wrongAnswers} wrong`;
...
function startTimer() {
    timerInterval = setInterval(() => {
        if (currentTime > 0) {
            ...
            clearInterval(timerInterval);
            document.getElementById('feedback').textContent = `Time's up! Correct answer was: ${currentExpression.answer}`;
            wrongAnswers++;
            document.getElementById('status').textContent = `Score: ${correctAnswers} correct | ${wrongAnswers} wrong`;
            document.getElementById('answer').value = '';
            document.getElementById('answer').focus();
